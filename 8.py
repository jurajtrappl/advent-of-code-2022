...
def is_tree_visible(row, col):
    left_max, right_max, top_max, bottom_max = list(map(max, neighbours(row, col)))
...
def first_part():
    ...
    interior_visible_count = np.sum([[is_tree_visible(row, col) for col in range(1, N - 1)] for row in range(1, N - 1)])
