...
def parse_input():
    def parse_stacks_of_crates(raw_stacks):
        horizontal_stacks = np.array([np.array(list(line))[CRATES_INDICES] for line in raw_stacks.split(os.linesep)[:-1]])
        stacks = [deque([crate for crate in horizontal_stacks[:,col]]) for col in range(NUM_OF_STACKS)]
...
def rearrangement_procedure(input, move_crates_func):
    ...
    for cmd in commands:
        stack_from, stack_to = crates_stacks[cmd.start - 1], crates_stacks[cmd.end - 1]
