...
class Command:
    ...
    def add(self, output_line: str) -> None:
        ...
        self.output.append(Directory(parts[1]) if parts[0] == DIR_START else File(
            name=parts[1], size=int(parts[0])))
    ...
    for command in program[1:]:
        ...
        if command.name == CommandName.CHANGE_DIR.value:
            if command.arg == PARENT_DIR:
                ...
                new_dir = [
                    child_dir for child_dir in current_dir.content if child_dir.name == command.arg][0]
                current_dir = new_dir
...
def calculate_dir_size(dir):
    ...
    while q:
        ...
        child_files = filter(lambda item: isinstance(
            item, File), current_dir.content)
        size += sum([f.size for f in child_files])

        child_dirs = list(filter(lambda item: isinstance(
            item, Directory), current_dir.content))
        q.extend(child_dirs)
